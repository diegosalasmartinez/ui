...
export default function BasicAlert({ title, message, type, icon, closable, children }: BasicAlertProps) {
  ...
  function getAlertStyle(): string {
    return type === 'success' ? 'bg-green-50 dark:bg-green-50 border-2 border-green-300'
      : type === 'info' ? 'bg-sky-50 dark:bg-sky-50 border-2 border-sky-300'
      : type === 'warning' ? 'bg-amber-50 dark:bg-amber-50 border-2 border-amber-300'
      : type === 'error' ? 'bg-rose-50 dark:bg-rose-50 border-2 border-rose-300' : ''
  }
  ...
  function getHoverStyle(): string {
    return type === 'success' ? 'hover:bg-green-200'
      : type === 'info' ? 'hover:bg-sky-200'
      : type === 'warning' ? 'hover:bg-amber-200'
      : type === 'error' ? 'hover:bg-rose-200' : ''
  }

  function getIcon(): React.ReactElement {
    return type === 'success' ? <SuccessIcon fill='none' color={getTextStyle()} />
      : type === 'info' ? <InfoIcon fill='none' color={getTextStyle()} />
      : type === 'warning' ? <WarningIcon fill='none' color={getTextStyle()} />
      : type === 'error' ? <ErrorIcon fill='none' color={getTextStyle()} /> : <></>
  }
