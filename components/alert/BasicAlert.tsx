...
export default function BasicAlert({ title, message, type, children }: BasicAlertProps) {
  function getAlertStyle(): string {
    return type === 'success'
      ? 'bg-green-100 dark:bg-green-50 border-2 border-green-300'
      : type === 'info'
      ? 'bg-sky-100 dark:bg-sky-50 border-2 border-sky-300'
      : type === 'warning'
      ? 'bg-amber-100 dark:bg-amber-50 border-2 border-amber-300'
      : type === 'error'
      ? 'bg-rose-100 dark:bg-rose-50 border-2 border-rose-300'
  ...
  return (
    <div className={cn('flex flex-col justify-between', 'rounded-lg p-2 mb-1', getAlertStyle())}>
      <span className={cn('ml-2 text-lg font-bold', getTextStyle())}>{title}</span>
      <span className={cn('ml-2 pb-1 text-base', getTextStyle())}>{children ?? message}</span>
    </div>
  )
}
