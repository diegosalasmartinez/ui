...
export default function BasicNotification({ title, message, type, icon, children }: BasicNotificationProps) {
  const [isActive, setIsActive] = useState(true)

  function getAlertStyle(): string {
    return type === 'success'
      ? 'bg-green-50 dark:bg-green-50 border-l-4 border-green-300'
      : type === 'info'
  ...
  return (
    <>
      {isActive && (
        <div className={cn('flex flex-row rounded-lg p-2 absolute min-w-fit right-3 ml-3 animate-bounce', getAlertStyle())}>
          ...
          <button
            className={cn('p-1 mr-1 ml-3 rounded-full self-center cursor-pointer', getHoverStyle())}
